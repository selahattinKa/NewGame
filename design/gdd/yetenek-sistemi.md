...
> **Last Updated**: 2026-06-30 (savas-sistemi.md yeniden adlandırma + AC #5 magic hasar düzeltmesi)
...
| Büyücü | Büyü Dalgası | 4 tur | ATK × `skill_aoe_multiplier` (0.75) tüm düşmanlara ayrı ayrı | Tüm düşmanlar | **Büyü** |
...
`normal_damage = max(1, floor(effective_ATK × 1.0 - floor(effective_DEF / defense_reduction_factor)) × [crit])`
...
`heavy_damage = max(1, floor(effective_ATK × skill_atk_multiplier - floor(effective_DEF / defense_reduction_factor)) × [crit])`

`defense_reduction_factor` sınıfın hasar türüne göre Formül 2 ile aynı kuralı izler (fiziksel=2, büyü=4).

| Değişken | Sembol | Tip | Aralık | Açıklama |
|----------|--------|-----|--------|----------|
| ATK çarpanı | skill_atk_multiplier | float | 2.0 | Registry sabiti (kaynak: savas-sistemi.md) |
| Efektif ATK | effective_ATK | int | 15–599 | Pipeline çıktısı |
...
- Savaş Sistemi GDD: cooldown sistemi entegre edildi ✓ (eski enerji sistemi kaldırıldı)
...
| Ağır Saldırı (Slot 1) | Güçlü slash + darbe dalgası + sınıf rengi + ekran hafif sarsıntısı | 0.8s | MVP |
...
| Pet — Büyü Dalgası | Sahneyi kaplayan renk dalgası, tüm düşmanlara aynı anda | 1.0s | MVP |
...
4. **GIVEN** Saldırgan pet (ATK=35, CD=4) yeteneği kullanılırsa, **WHEN** CD=0 olduğunda, **THEN** hasar = max(1, floor(35×2.0 - floor(enemy_DEF/2))).

5. **GIVEN** Büyücü pet Büyü Dalgası (ATK=35, multiplier=0.75, büyü hasarı) vs 3 düşman (DEF: 35, 15, 17), **WHEN** yetenek kullanılırsa, **THEN** boosted=floor(35×0.75)=26; her hedefe: floor(26-floor(35/4))=floor(26-8)=**18**, floor(26-floor(15/4))=floor(26-3)=**23**, floor(26-floor(17/4))=floor(26-4)=**22**. Toplam=**63**.
...
4. **Komutan modu fallback süresi ne kadar?** Şu an "tur sonunda fallback" — tur kaç saniye sürer? 1x hızda ~2 saniye: oyuncu 2 saniyede basmazsa Normal Saldırı mı tetiklenir? → Savaş Sistemi hız tablosuyla koordine edilmeli.
