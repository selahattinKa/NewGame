...
**Negatif fantezi (kaçınılacak)**: "Boş kat" — oyuncunun bir katı temizleyip hiçbir şey kazanmaması. Cömert Zindan sütunu bunu yasaklar: her kat en azından altın ve bir miktar malzeme düşürür. Ayrıca "hayal kırıklığı loot'u" — oyuncunun 50 kat boyunca aynı F tier canavarları görmesi. Çeşitlilik ve nadir sürprizler sisteme serpiştirilmeli.
...
Zindan katları 1, 2 ve 3'te (normal katlar) canavar düşmesi %100 garantidir. Nadirlik normal weighted random ile belirlenir (Kural 5) — genellikle F tier ama nadir sürpriz mümkündür. Bu kural "her sandık heyecanlı" fantezisini onboarding'den itibaren somutlaştırır.
...
| Kademe | Düşme Ağırlığı | Normalize Olasılık |
|--------|----------------|-------------------|
| F | 1.000 | %58.6 |
| D | 0.500 | %29.3 |
| C | 0.150 | %8.8 |
...
| Boss Düşen Pet Kademe | Düşme Oranı | Not |
|----------------------|-------------|-----|
| B Tier Boss | %5 | Her yenilgide %5 şans |
| A Tier Boss | %3 | Çok nadir |
| S/SS Tier Boss | %1 | Neredeyse efsanevi |
...
| `monster_hard_pity` | 10 kat | Hard cap — 10 ardışık canavarssız normal kattan sonra garanti F tier canavar |
...
Hard pity: 10 ardışık normal katta canavar düşmediyse, 11. normal katta F tier canavar garanti düşer. Hard pity sadece normal katları sayar — boss katları (base_rate %35) ayrı değerlendirilir. Hard pity tetiklendiğinde canavar, bölge havuzundan F tier kademeyle rastgele seçilir.

**10b — C Pity (Kademe Koruma)**

| Parametre | Değer | Açıklama |
|-----------|-------|----------|
| `rare_pity_increment` | +%2 / düşen canavar | C+ düşmeyen her canavar düşmesinde C ağırlığı artar |
| `rare_pity_cap` | 20 | Maksimum rare_pity_counter değeri — counter 20'de clamp'lenir |
| `rare_pity_reset` | C+ düştüğünde | C veya üstü düşünce sıfırlanır |

Çalışma şekli: 10 canavar düştü hepsi F/D → rare_pity_counter=10, C ağırlığı 0.15 + 10×0.02 = 0.35. Counter 20'de clamp'lenir: max adjusted_C = 0.15 + 20×0.02 = 0.55, total_weight = 2.106, P(C) = %26.1, P(B) = %1.9, P(A) = %0.47, P(S) = %0.24, P(SS) = %0.05. B/A/S/SS dilüsyonu sınırlı tutulur.

C pity sadece **aktif** zindanda geçerlidir. İdle'da düşen F/D canavarlar rare_pity_counter'ı artırmaz — idle kademe tablosu bağımsız çalışır.
...
| **Hard pity kontrolü** | Random rulo sonrası canavar düşmedi | `hard_pity_counter` 10'a ulaştıysa → garanti F tier canavar. Sadece normal katları sayar. |
...
| Toplam ağırlık | total_weight | float | 1.706–2.106 | Tüm ağırlıkların toplamı. Base 1.706; C pity aktifken dinamik olarak yeniden hesaplanır (bkz. Formül 4) |
| Olasılık | P(r) | float | 0.006–0.588 | Normalize olasılık |

**Çıktı**: F=%58.6, D=%29.3, C=%8.8, B=%2.3, A=%0.59, S=%0.29, SS=%0.06

**Örnek**: `rand()=0.72` → Kümülatif: F [0, 0.586), D [0.586, 0.879) → 0.72 D aralığında → D seçilir.
...
**Hard pity**: `if hard_pity_counter >= monster_hard_pity → garanti F tier canavar`
...
| Hard pity eşiği | monster_hard_pity | int | 10 | Bu değere ulaşınca garanti F tier |
...
| Kademe | Ağırlık | Normalize Olasılık |
|--------|---------|-------------------|
| F | 0.700 | %69.9 |
| D | 0.240 | %24.0 |
| C | 0.050 | %5.0 |
...
### Formül 4: C Pity (Sadece C Ağırlığına, Cap'li)

`adjusted_rare_weight = base_rare_weight + min(rare_pity_cap, rare_pity_counter) × rare_pity_increment`

| Değişken | Sembol | Tip | Aralık | Açıklama |
|----------|--------|-----|--------|----------|
| C temel ağırlığı | base_rare_weight | float | 0.15 | Canavar Veritabanı (C tier) |
| C pity artışı | rare_pity_increment | float | 0.02 | F/D düştükçe |
| C pity sayacı | rare_pity_counter | int | 0–20 | C+ düşmeyen canavar sayısı, 20'de clamp'lenir |
| C pity cap | rare_pity_cap | int | 20 | Counter bu değerde durur |
| Düzeltilmiş ağırlık | adjusted_rare_weight | float | 0.15–0.55 | C ağırlığı artar, max 0.55 |

B (0.04), A (0.01), S (0.005) ve SS (0.001) ağırlıkları sabit kalır. Sadece C ağırlığı artar → toplam ağırlık büyür → C oranı yükselir, diğerleri orantılı düşer. Cap sayesinde B/A/S/SS dilüsyonu sınırlı tutulur.

**Cap'teki değerler** (rare_pity_counter=20): adjusted_C = 0.15 + 20×0.02 = 0.55. Total = 1.00+0.50+0.55+0.04+0.01+0.005+0.001 = 2.106. P(C) = %26.1, P(B) = %1.9 (base %2.3'ten), P(A) = %0.47 (base %0.59'dan). S/SS dilüsyonu da sınırlı, sistem stable.

**Örnek**: 10 F/D tier canavar düştü → adjusted_C = 0.15 + 10×0.02 = 0.35. Yeni total = 1.00+0.50+0.35+0.04+0.01+0.005+0.001 = 1.906. P(C) = **%18.4** (base %8.8'den).

C pity sadece **aktif** zindanda geçerlidir (bkz. Formül 3b).
...
**Ekonomi kontrolü**: ~8.500 altın → F tier canavar Lv1→11 (10 seviye atlama, maliyet ~7.130 altın) veya C tier canavar Lv1→9 (8 seviye, maliyet ~7.250×1.5 rarity_cost = ~5.880 altın). "Bir oturumda 1-2 güçlendirme" garantisi **sağlanıyor**. ~4.45 canavar/oturum ile koleksiyon ilerlemesi hızlı ve tatmin edici.
...
- **If pity_counter soft cap'e ulaşır (%45) ve hâlâ canavar düşmezse**: Oran %45'te kalır. Hard pity devreye girer: 10 ardışık canavarssız normal kattan sonra F tier canavar garanti düşer. Boss katları hard pity sayacına dahil değildir. Hard pity tetiklendiğinde bölge havuzundan rastgele F tier canavar seçilir ve tüm pity counter'lar sıfırlanır.
...
- **If idle'da F tier canavar düşerse ve envanter doluysa**: Canavar beklemeye alınır, aynı aktif kural.
...
| `rare_pity_increment` | 0.02 | 0.01–0.04 | C çok sık düşer → nadir hissetmez | C ulaşılamaz → koleksiyon stagnates |
| `rare_pity_cap` | 20 | 10–30 | Çok düşük → C pity etkisiz | Çok yüksek → B/A/S/SS aşırı dilüe olur |
...
| Zindan sonu rapor — canavar kartı açılışı | Kart döner, kademe arka plan rengi, C+ parlama | MVP |
...
| Canavar düşmesi (F/D tier) | Kısa "pop" ses | MVP |
| Canavar düşmesi (C+) | Artan gerilim tonu + "shimmer" ses (kademe arttıkça daha epik) | MVP |
...
5. **GIVEN** kademe ağırlıkları {F:1.000, D:0.500, C:0.150, B:0.040, A:0.010, S:0.005, SS:0.001} ve total_weight=1.706 (base, C pity=0), **WHEN** P(kademe) hesaplanırsa, **THEN** P(F)=0.586±0.001, P(D)=0.293±0.001, P(C)=0.088±0.001, P(B)=0.023±0.001, P(A)=0.006±0.001, P(S)=0.003±0.001, P(SS)=0.001±0.0005. total_weight C pity aktifken dinamik olarak yeniden hesaplanır.
...
8. **GIVEN** oyuncu art arda 4 kez F/D tier canavar almış (rare_pity_counter=4), **WHEN** kademe seçimi yapılırsa, **THEN** C ağırlığı = 0.15 + 4×0.02 = **0.23**; B (0.04), A (0.01), S (0.005) ve SS (0.001) değişmez.
...
13. **GIVEN** boss katı tamamlanıyor, **WHEN** boss canavar drop kontrolü yapılırsa, **THEN** oranlar: B Tier boss=%5, A Tier=%3, S/SS Tier=%1. Bu rulo normal canavar rulosundan bağımsız çalışır.
...
14d. **GIVEN** idle'da canavar düşer, **WHEN** kademe seçilirse, **THEN** idle kademe tablosu uygulanır: F=0.700, D=0.240, C=0.050, B=0.008, A=0.002, S=0.001 (SS idle'da düşmez; toplam~1.001).
...
16. **GIVEN** oyuncunun canavar envanteri dolu, **WHEN** yeni canavar düşerse, **THEN** canavar bekleme alanına eklenir ve bildirim gösterilir. Bekleme süresi kademesine göre değişir: F/D=7 gün, C/B/A=14 gün, S/SS=30 gün (bkz. Canavar Toplama GDD Kural 4). Süre dolduğunda bekleme alanındaki ilgili canavar otomatik satılır. Satış fiyatı Canavar Toplama GDD Formül 2 ile belirlenir.
...
18. **GIVEN** oyuncu 10 katlık zindanı tamamlıyor, **WHEN** zindan sonu raporu açılırsa, **THEN** loot artan heyecan sıralamasıyla gösterilir: altın → malzeme → iksir → elmas → canavarlar (F tier→SS tier) → ilk temizleme bonusu. Düşmeyen kategoriler atlanır.
...
20. **GIVEN** 10 ardışık normal katta canavar düşmemiş (hard_pity_counter=10), **WHEN** 11. normal katta loot roll yapılırsa, **THEN** F tier canavar garanti düşer (bölge havuzundan rastgele).

21. **GIVEN** rare_pity_counter=20, **WHEN** F tier canavar düşer, **THEN** rare_pity_counter 20'de kalır (clamp), artmaz. adjusted_C_weight = 0.15 + 20×0.02 = 0.55.
...
31. **GIVEN** rare_pity_counter=12, **WHEN** C veya üstü kademede canavar düşerse, **THEN** rare_pity_counter=0. VE F/D düşerse rare_pity_counter=13 olur.
